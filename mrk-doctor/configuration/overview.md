...
Dashboard credentials generated by `/doctor dashboard setup` become active immediately. Restart after changing bind addresses, ports, enabled state, or monitoring settings. `/doctor reload` is not a substitute for a full restart for those connection and lifecycle changes.
